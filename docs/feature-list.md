...
### Streaming mode setting (Hybrid / None / Full / Chunked)
Global (not per-project) setting with four modes: `"hybrid"` (default — most chat surfaces stream live, but Claude Subscription's Sonnet/Opus and sub-agent cards do not), `"none"` (every surface delivers one complete response), `"full"` (everything streams token-by-token, including Claude Subscription Sonnet/Opus and Playground), `"chunked"` (like full, but text arrives in larger blocks — a coarse ~500ms flush + newline-flush, so code/lists stream line-by-line while prose updates ~twice a second, with far fewer re-renders). Applies wherever full does. Two helpers centralize the behavior: `isLiveStreamingMode(mode)` (full||chunked) and `streamingFlushArgs(mode)` (chunked → `[500, true]` for `createThrottledAccumulator`). **General Chat is excluded from the global setting** — it has its OWN three-mode streaming setting (`"chunked"` **(default)** / `"full"` smooth / `"none"`, no hybrid), in a dialog opened by the gear icon in the General Chat header, key `generalChatStreamingMode` (read by `getGeneralChatStreamingMode` in `agents/streaming-mode.ts`, surfaced by `components/general-chat/general-chat-settings.tsx`).
**How chunked is applied per surface (heterogeneous — two streaming mechanisms):**
- **PM project chat + Quick Chat** (`engine.ts`): chunked on the FRONTEND token buffer (`stores/chat-event-handlers.ts` — 500ms + newline instead of 32ms), covering both the CLI and streamText branches without touching the intricate PM retry loop. `engine.ts` only needed `isFullStreaming = isLiveStreamingMode(streamingMode)` so the CLI branch streams in chunked. Retract is handled for free (`onStreamReset` already clears the buffer). The frontend re-reads the mode on load + on `agentdesk:settings-changed` (dispatched by the Streaming settings page on save).
- **Widget chats** (`collections/chat.ts`, `rpc/dashboard.ts`, `rpc/dashboard-agent.ts`, `rpc/council.ts`, `rpc/freelance-chat.ts`, `rpc/skills-search-chat.ts`): chunked on the BACKEND — CLI-branch accumulators get `streamingFlushArgs`, and each streamText branch gets a chunked-only `sbTextAcc` accumulator that batches its raw delta broadcasts.
- **Sub-agent cards / Playground / General Chat** (`agent-loop.ts` `pushLiveDelta`): the accumulator's flush args come from `streamingFlushArgs(streamingMode)` centrally.
**Key files:**
- `src/mainview/pages/settings/streaming.tsx` — `StreamingSettings`, `StreamingMode` type (4 options)
- `src/bun/agents/streaming-mode.ts` — `getStreamingMode` (global) + `getGeneralChatStreamingMode` (General Chat's own) + `isLiveStreamingMode` / `streamingFlushArgs` / `CHUNKED_FLUSH_MS`
- `src/mainview/stores/chat-event-handlers.ts` — PM chat frontend token buffer (chunked = 500ms + newline)
- Persisted via generic `rpc.getSetting`/`saveSetting("streamingMode"/"generalChatStreamingMode", value, "ai")` — no dedicated RPC handler
**Data:** `settings` table, keys `streamingMode` + `generalChatStreamingMode`, category `ai`
**Watch for:** enforcement is spread across engine.ts (via the frontend buffer), the widget backends, and agent-loop.ts — the UI copy must stay accurate to what those actually do; a common drift point. `hybrid`/`full`/`none` behavior is byte-identical to before — `chunked` is purely additive/opt-in.
...
## Project Chat Page, Dashboard, Quick Chat, General Chat & Composer Features
...
Shared hook wrapping `webkitSpeechRecognition`/`SpeechRecognition` (Chromium/Blink only — absent on macOS WKWebView, feature-detected via `supported`). Appends live transcript after whatever text was already in the box; stopped automatically right before a send so the mic doesn't keep listening in the background. `appendWithSeparator` joins every piece of a growing transcript (consecutive final results, consecutive interim results, and the join against pre-existing box text) with a space unless one side already supplies the boundary — found via an Ambient Mode log review: consecutive recognizer results landing in separate `onresult` callbacks concatenated with no space at all ("Nothing else." + "thank you" → "Nothing else.thank you"), since the Web Speech API gives no guarantee a result carries its own leading/trailing whitespace. Fixed once, shared by every caller (this hook also backs the normal per-project chat's dictation button, not just Ambient Mode).
**Key files:**
- `src/mainview/lib/use-voice-input.ts` — `useVoiceInput`, `appendWithSeparator`
...
### General Chat (standalone "Assistant" agent — ChatGPT-style, no project)
A new sidebar nav item (above Playground) opens `/general-chat` as a normal embedded route inside the main `AppShell` — unlike Quick Chat, no second `BrowserWindow`, and unlike a project's chat, no PM/kanban/plan-approval/Constitution at all. Backed by a dedicated built-in agent, `general-chat-assistant` ("Assistant"), hidden the same way `playground-agent`/`issue-fixer`/`freelance-expert` are: excluded from the Agents page listing **and** from the PM's own "Sub-Agents Available" table (`buildAgentsSection()`) so it can never be dispatched as a sub-agent either. It has no `run_agent`/`run_agents_parallel` tools and cannot delegate — every turn is handled by Assistant alone via `runInlineAgent`, called directly from a small dedicated orchestrator (mirrors the Playground's use of `runInlineAgent`, not the PM engine). Each conversation gets a fresh, isolated OS-temp-folder workspace (`{tmpdir}/agentdesk-general-chat/<conversationId>`) — never shared across conversations. Tool-call activity streams live (one call at a time, via the shared `ToolCallFeed`) but is **never persisted** — only the final user/assistant text of each turn is written to the DB (a flat `general_chat_messages` table, no parts table), so history reloads as plain markdown with no tool-call remnants. One deliberate exception: images from `generate_image`/`take_screenshot` are embedded into that final text (see "Generated images" below) so they survive a reload despite the no-parts-table design. A **Deep Research** toggle (replacing the Build/Plan Mode toggle main chat shows) makes Assistant ask a clarifying question before invoking the real `deep_research` tool, rather than researching a vague request blindly. Assistant has its own exclusive memory (`save_memory`/`recall_memory`/`delete_memory`, bound to `general_chat_memories` — never shared with the PM's `global_memories` or any project's `agent_memories`) and its own per-conversation, in-memory todo-list tools. MCP tools (e.g. `chrome-devtools_*`) are called directly, with no "delegate to a sub-agent" framing, since `runInlineAgent` merges MCP tools into any inline agent's tool map generically. Slash commands are restricted to `/clear`, `/compact`, `/fork`, `/mcp`, `/new` (no `/init`/`/preview`/`/info`, none of which apply without a real project; `/compact` hides itself below 50% estimated context utilization, same threshold `chat-input.tsx` uses).

Visually mirrors project chat's `ChatInput`/`TypingRow`: the "Thinking…" indicator (`AssistantTypingRow`) reuses the same rainbow-bordered-bubble/lightbulb/typewriter-reveal visual as `TypingRow`, with `<AgentAvatar name="general-chat-assistant">` in place of `TypingRow`'s hardcoded PM badge; a fixed-bottom "Responding…"/"Compacting conversation…" pill appears during a turn/compaction; the input bar is a pill-shaped bordered container (icons inline with the textarea) with file attach, attach-a-note (`AttachNoteModal`), the shared prompts library dropdown, a mic button (`VoiceInputButton`/`useVoiceInput`, the same hook project chat uses), an MCP status row + reconnect/disconnect dialog shown above it when servers are configured, and a small local context-usage indicator (~tokens / bar / %) next to the model selector.

**File/note attachments:** reuses `ChatInput`'s own `AttachmentFile`/`categorizeFile`/`processFiles`/`ACCEPT_ALL` (now exported for this purpose) and mirrors `chat-layout.tsx`'s attachment handling: each file is base64-encoded and saved via the existing `saveAttachment` RPC (`conversationId` in place of `projectId` — safe, since that param is only read in the fallback branch when the global workspace path isn't configured), folded into the same "implicit context" text block project chat uses (`<attached-file>` for text, a note telling the model to call `read_image`/`read_audio` for media — both tools are unsandboxed to any absolute path so they reach the saved file regardless of the assistant's own temp `workspacePath`; binary docs get a plain text note, same no-dedicated-tool limitation as project chat) prepended to the sent text. Unlike project chat's two-representation model (a separate optimistic-only `visibleContent` vs. the DB-persisted wrapped text), General Chat stores and displays the **same** wrapped string in both places, with its own local `extractAttachmentChips` stripping it back to plain chip labels at render time — no image-thumbnail/lightbox richness (a deliberate scope trim vs. project chat's `AttachmentPreviews`).

**Compaction — manual `/compact` AND automatic:** `compactConversation` (`orchestrator.ts`) summarizes everything but the most recent 10 messages into one condensed message via a single `generateText` call (no parts table to prune, unlike project chat's `summarizer.ts`), deletes the originals, and broadcasts `generalChatCompacted` so the page reloads. No separate summaries table — a compacted summary is just another `general_chat_messages` row and can itself be re-compacted later. `rpc/general-chat.ts`'s `compactGeneralChatConversation` (the `/compact` handler) is now a thin wrapper over it — the same function is also called **automatically**, mirroring project chat's own auto-compaction (`AgentEngine._runPMProcessing`/`triggerSummarization`): `sendMessage` checks a per-conversation `lastPromptTokens` map (updated from `runInlineAgent`'s real `result.tokensUsed.prompt` after each turn) against `getContextLimit(modelId, conversationId)` before loading `priorMessages` for the next turn, compacting first if over the limit. Failure is caught and logged, never blocking the turn.

**Generated images survive reload:** `generate_image`/`take_screenshot` results (raw base64, via the shared `extractImagePayload` helper) are captured in `sendMessage`'s tool-call callbacks and embedded into the persisted assistant message as `<generated-image mime="...">base64</generated-image>` blocks once the turn completes — the only way a generated image outlives the live-only tool-call stream, since `general_chat_messages` has no parts table. `GeneralChatBubble` strips these back into real `<img>` elements (click-to-zoom lightbox) at render time. Scoped to `generate_image`/`take_screenshot` only — `read_image` (viewing an existing attachment, not producing new output) is excluded.

**Per-message hover actions:** each bubble's hover row now mirrors project chat's `MessageBubble` exactly — user bubbles show Delete/Copy/Fork ("Fork from here")/timestamp (DOM order delete→copy→fork→timestamp under `flex-row-reverse`, so the visual order reads timestamp→fork→copy→delete, identical to `message-bubble.tsx`); assistant bubbles show Copy/Save to Collection/Retry (last message only)/Delete/timestamp/model id. **Delete** calls the new `deleteGeneralChatMessage` RPC and removes the row from `general_chat_messages`. **Fork from here** reuses the existing `forkGeneralChatConversation(id, upToMessageId)` RPC (already supported per-message, just previously only wired to a whole-conversation slash command) and switches the page to the new conversation. **Retry** deletes the last assistant row and resends the preceding user message's content as a brand-new turn via the same `sendToConversation` helper `handleSend` uses — it deliberately does NOT delete the old user row first, mirroring `chat-store.ts`'s `retryLastMessage` exactly (the prior user bubble stays, a fresh one is added alongside the regenerated reply). **Save to Collection** reuses the shared `SaveToCollectionModal` with a new `sourceType: "general_chat"` (added to `CollectionNoteSourceType`, no `sourceRef` — General Chat conversations aren't projects). **Model id** reads a new `metadata` column on `general_chat_messages` (JSON, `{modelId}`, set on assistant/compaction-summary rows only — mirrors project chat's `messages.metadata` convention) — `sendMessage`'s DB insert now uses explicit, pre-generated UUIDs for both the user and assistant rows instead of relying on the schema's `$defaultFn`, and the `generalChatComplete` broadcast carries those real ids (`userMessageId`/`assistantMessageId`) plus `modelId` so the frontend can patch its optimistic bubbles to the real persisted ids in place (no reload round-trip) — needed because Delete/Fork/Retry all operate on the real DB row id, not a client-generated placeholder.
**Header bar:** a new header row above the messages (mirrors `chat-layout.tsx`'s exactly, minus Focus mode and the activity-pane toggle — General Chat has no sub-agents/activity pane to hide): a `PanelLeft` button now makes the conversation sidebar toggleable (previously always-visible with no toggle; the sidebar is now `w-[220px]` with the same animated width-collapse as `chat-layout.tsx`, was `w-64`/hard show-hide), the active conversation's title (click to copy its id), a centered "Clear Chat" button (click-again-to-confirm, reuses the existing `/clear` handler), "New conversation" (`SquarePen`), font-size zoom controls (`useConvFontSize("conv-font-size-general-chat")` — its own independent zoom level from project chat's), "Search messages" (reuses `MessageSearch` as-is — its prop shape is already generic enough for `DisplayMessage[]`; matched text highlights via a `<mark>` wrapper, `SearchHighlight`/`highlightChildren` duplicated from `message-bubble.tsx` and threaded into a new `buildMdComponents(query)` function replacing the old static `MD_COMPONENTS` object), and "Export as markdown" (client-side `Blob` download, logic duplicated from `chat-layout.tsx`'s `handleExportMarkdown`).

**Auto-title from first message:** conversations now rename themselves from the first user message, same as project chat — `orchestrator.ts`'s `sendMessage` mirrors `engine.ts`'s `autoTitleConversation` (same 40-char truncation rule, no source-channel prefix since General Chat has no Discord/WhatsApp/email surface) and fires it before the turn starts (not after it completes) so the title lands immediately. A new `generalChatConversationRenamed` broadcast (`{conversationId, title}`) lets the frontend patch its `conversations`/`archivedConversations` state in place — needed because `sendGeneralChatMessage` is fire-and-forget, so the existing post-send `reloadConversations()` call would otherwise race the rename and often miss it.

**Fixed: shell approval requests were invisible (turn would silently hang).** `run_shell`'s approval gate (`engine-manager.ts`'s `installShellApprovalHandler`) broadcasts `shellApprovalRequest` into the shared, global `useChatStore.shellApprovalRequests` array exactly like it does for project chat — General Chat's `runInlineAgent` call already passes `projectId: conversationId`, so the request lands correctly keyed. The gap was purely on the render side: nothing on `general-chat.tsx` ever read that array or rendered `ShellApprovalCard`, so a pending request sat invisibly in the store with no way to respond — the blocked `run_shell` tool call just hung until the backend's own approval timeout auto-denied it. Fixed by subscribing to the same store (filtered to the active conversation, memoized like `chat-layout.tsx`'s own `shellApprovalRequests`) and rendering `ShellApprovalCard` in the same position (between the message list and the input area).

**Fixed: cross-project shell-approval toast fired for General Chat's own open conversation.** The global `CrossProjectApprovalToast` (mounted once in `AppShell`) suppresses its toast when `useChatStore.activeProjectId` already matches the request — works for project chat/Quick Chat (both call `setActiveProject`), but General Chat never did, so the toast fired even when its own conversation (whose broadcasts use `conversationId` in place of `projectId`) was the one already open, redundant with the inline `ShellApprovalCard`. Fixed with a new `activeGeneralChatConversationId` field on `useChatStore`, kept in sync by `general-chat.tsx` and checked as a second suppression condition.

**Fixed: "Thinking…" disappeared the instant a tool call started.** `AssistantTypingRow`'s render condition included `toolCalls.size === 0`, so it unmounted as soon as the first tool call began — unlike project chat's `TypingRow`, which stays visible alongside the tool-call feed the whole time the agent hasn't produced text yet (`message-list.tsx`'s `showTypingDots` has no tool-call-count dependency at all). Fixed by dropping that condition so both render together, matching PM chat.

**Fixed: leaving/refreshing mid-turn lost the "still working" state.** `isSending`/streaming state was pure local React state, reset to empty on every mount — unlike project chat, whose `AgentEngine` lives in the Bun backend process and survives a webview refresh (`project.tsx`'s `syncRunningAgents` re-queries it on mount). So navigating away and back, or refreshing, while the Assistant was still generating dropped the Stop button/busy indicator even though the backend turn (and its `orchestrator.ts` `abortControllers` entry) was still genuinely running — the final reply still arrived correctly, but everything in between looked broken. Fixed with a new `getGeneralChatStatus(conversationId)` RPC (wraps the existing `isGeneralChatRunning`), checked in `general-chat.tsx`'s per-conversation load effect: if the backend reports a turn in flight, `isSending` is set `true` (only ever set `true` from this check, never `false`, so it can't race a send that starts locally while the lookup is in flight). Deliberately does NOT reconstruct the tool-call history made before the reload — matches General Chat's existing "no tool-call persistence" design; a reconnect just shows the generic thinking indicator + Stop button until the turn completes, at which point the existing live listeners take over normally.

**Fixed: the global Streaming setting had no effect on General Chat, in any mode.** Its live text was never actually rendered — `general-chat.tsx` was wired to `InlineAgentCallbacks.onTextDelta`, which `agent-loop.ts` (the `runInlineAgent` mechanism General Chat/Playground/sub-agent cards all share) never calls at all; only PM chat's/the six widget-chat surfaces' own separate direct-`streamText` loops call it. The real live-text signal under Full Streaming is a `type: "text"` message part, created/updated via `pushLiveDelta` — General Chat's part-created handler only ever handled `tool_call`, and the corresponding part-updated broadcast had no listener. Fixed by handling `"text"` parts too (tracking the live part's id, plus a committed-prefix ref so a later turn-step's fresh part id appends rather than blanking out what's already shown) and wiring up the previously-unused `generalChatPartUpdated` listener. Separately: General Chat has no sub-agent-card concept, so Hybrid mode's entire reason to differ from Full — "sub-agent cards update per step, not live" — doesn't apply to it; `orchestrator.ts` now maps the global "hybrid" setting to `streamingModeOverride: "full"` specifically for its own turn (an explicit "none" is left as a real opt-out). Settings → AI → Streaming's Hybrid description was updated to spell out this General Chat exception.

**Top nav parity — title.** The global top nav showed the bare app name for `/general-chat` (missing from `PAGE_TITLES`). Added `"General Chat"`. (A per-conversation workspace folder icon was also briefly added here, then reverted the same session once General Chat became workspace-less — see below.)

**Fixed: `write_file`/`edit_file`/etc. still worked after being removed from Assistant's tools.** `agent-loop.ts` was merging `trackedFileTools` (the real, tracked implementation of the read/write/edit/multi_edit/patch/append/delete/move file family) into every agent's tool set unconditionally — regardless of what `agent_tools` actually granted. Confirmed live: Assistant wrote a real file to its hidden temp folder despite zero `write_file` rows in its `agent_tools`. Pre-existing bug (not introduced by the tool cut below) — masked until now because the only agents with a restricted file grant were the 3 read-only agents, which get a separate blunt strip. Fixed by making that merge an overlay (only replaces a tool the agent's own `agent_tools` already granted, never adds one it didn't) — also fixes the same silent gap for any custom agent with `write_file` unchecked in Settings.

**Redesigned: Assistant is workspace-less and has no `run_shell` — reads/answers directly, ChatGPT-style, never creates or manages files.** `defaultAgentTools["general-chat-assistant"]` (`seed.ts`) cut from 32 file/process/shell-capable tools down to 12: `find_skills`, `generate_image`, `http_request`, `read_audio`, `read_file`, `read_image`, `read_skill`, `read_skill_file`, `sleep`, `validate_skill`, `web_fetch`, `web_search` (plus the auto-added `request_human_input`) — no write/edit/delete/directory-browsing tools, no shell, no `take_screenshot`/`environment_info`. A new migration (`v63_assistant-workspace-less.ts`) deletes all existing `agent_tools` rows for the agent so existing installs actually lose the removed tools too (the per-boot tool backfill is additive-only — it never removes rows on its own). Its system prompt's `## Your workspace` section is gone, and `## Style` now says to give every answer directly in chat, in full — never claim it'll "save" or "create" a file. Under the hood it still gets a real, hidden per-conversation temp path (unchanged) so `read_file`'s safety boundary and `generate_image`'s output still work — `agent-loop.ts`'s generic "Workspace: ..." system-prompt injection (used by every `runInlineAgent` caller) is now skipped specifically for `agentName === "general-chat-assistant"`, so the model is never told about it. UI: `ModelSelector`'s "Shell: Ask/Auto" toggle is now hidden for General Chat (new `hideShellApproval` prop — meaningless with no `run_shell` tool), and the `ShellApprovalCard` block General Chat briefly grew (to fix a shell-approval-requests-invisible bug a few fixes above) was removed outright — Assistant can never trigger one anymore. `log_decision` is also now excluded for Assistant — it's granted to any agent with a workspace regardless of `agent_tools` (never a row in anyone's `agent_tools`, by design, so a plain `defaultAgentTools` change couldn't touch it either), same fix pattern as the "Workspace: ..." prompt injection: gated on `agentName !== "general-chat-assistant"`. A model claim of LSP capability was initially (wrongly) written off as hallucination — checking only `agent_tools` missed that plugin-registered tools (LSP Manager's `lsp_diagnostics`/`lsp_hover`/`lsp_definition`/`lsp_references`/`lsp_document_symbols`) live under a *prefixed* runtime name (`plugin__lsp_manager__lsp_diagnostics`, etc.) entirely outside the `agent_tools` system, merged into every agent unconditionally (same tier as MCP tools, no per-agent grant/deny at all). Confirmed live (LSP Manager was genuinely enabled) that Assistant really did have working LSP tools — not hallucinated. Fixed by skipping `pluginTools` entirely for `agentName === "general-chat-assistant"`, so no current or future plugin can reopen this.

**Renamed: internal agent identifier `assistant` → `general-chat-assistant`, and dropped `take_screenshot`/`environment_info` from its tool grant.** The bare name `assistant` was too easy for a user to also give a custom agent (Settings → Agents accepts any string) — a collision would wrongly hand that custom agent every `agentName === "assistant"` special-case above. Renamed everywhere it's checked (`agent-loop.ts`, `prompts.ts`, `rpc/agents.ts`, `general-chat/orchestrator.ts`, `seed.ts`). A new migration, `v64_general-chat-assistant-rename.ts`, renames the existing DB row in place (`UPDATE`, not delete+reinsert — keeps its id and `agent_tools` rows attached), scoped to `is_builtin = 1` and skipped if the target name is already taken, so it can't touch an actual user-created custom agent. `take_screenshot`/`environment_info` dropped in the same pass (no screenshot/env-info use case for a workspace-less chat agent) — `general-chat/orchestrator.ts`'s `IMAGE_OUTPUT_TOOLS` set (embeds a tool's base64 output into the persisted message) now only lists `generate_image`.

**Live tokens/s + input auto-focus (parity fixes):** `GeneralChatContextIndicator` now shows a live "N tokens/s" readout next to the context bar, matching `ContextIndicator`'s inline variant — wired via `InlineAgentCallbacks.onStreamPerformance`, a generic per-call callback `runInlineAgent` already invokes for every caller (previously only consumed by the PM engine's `chat-event-handlers.ts`/`useChatStore`), forwarded through a new `generalChatStreamPerformance` broadcast (`{conversationId, tokensPerSecond, timeToFirstOutputMs}`) into a local `liveTokensPerSecond` state — deliberately local, not the shared store, same reasoning as the rest of this indicator. Resets to 0 on conversation switch and on each new send. Separately, the message textarea's auto-focus effect was gated only on `isSending`, which is already `false` on first mount — but the textarea is still `disabled` at that point (no `activeConversationId` yet, set async by `reloadConversations()`), so the very first `focus()` call was a no-op with no later retry. Fixed by adding `activeConversationId` to the effect's dependency array/condition, mirroring `chat-input.tsx`'s own `[disabled]`-keyed focus effect.
**Key files:**
- `src/mainview/pages/general-chat.tsx` — `GeneralChatPage`, its own local component state (not `useChatStore`), reuses `ConversationSidebar` (generalized — `projectId` is now optional), `ToolCallFeed`, `ModelSelector` (new `hideBuildPlanToggle` prop, rendered below the input pill like `chat-layout.tsx`), `VoiceInputButton`/`useVoiceInput`, `MessageSearch`, `useConvFontSize`, and `useInputPopover`/`SLASH_COMMANDS` filtered to 5 commands; its own `AssistantTypingRow`, `GeneralChatContextIndicator`, `GeneratedImages`, MCP status row/dialog, header bar, and `GeneralChatBubble`'s hover action row (`handleDeleteMessage`/`handleForkFromMessage`/`handleRetry`/`sendToConversation`)
- `src/mainview/components/general-chat/deep-research-toggle.tsx` — `DeepResearchToggle`, renders in place of the Build/Plan toggle
- `src/bun/general-chat/orchestrator.ts` — `sendMessage`/`stopGeneralChatGeneration`/`isGeneralChatRunning`/`resolveProviderConfig`/`compactConversation` (auto-compaction + the `/compact` RPC both call this one function); `src/bun/general-chat/paths.ts` — `getGeneralChatWorkspacePath`
- `src/bun/agents/tools/general-chat-memory.ts` / `general-chat-todos.ts` — Assistant-exclusive tools, injected via `extraTools`, never in the shared `toolRegistry`
- `src/bun/agents/prompts.ts` — `getAssistantSystemPrompt` (dedicated dispatch branch in `getAgentSystemPrompt`, omits Constitution/kanban/project-context)
- `src/bun/rpc/general-chat.ts` + `src/shared/rpc/general-chat.ts` — every method prefixed `generalChat`/`GeneralChat` (Electrobun's RPC schema is one flat namespace; the semantically-matching bare names already exist for project chat), including `compactGeneralChatConversation`/`deleteGeneralChatMessage`/`getGeneralChatStatus`
**Data:** `general_chat_conversations`, `general_chat_messages` (flat — no parts table; `metadata` column added in migration `v62_general-chat-message-metadata.ts`), `general_chat_memories` (migration `v61_general-chat.ts`) — all fully independent of `projects`/`conversations`/`messages`. `agent_tools` rows for the agent are wiped and re-seeded by `v63_assistant-workspace-less.ts` and again by `v64_general-chat-assistant-rename.ts` (which also renames the `agents` row itself from `assistant` to `general-chat-assistant` in place, preserving its id).
**Watch for:** `runInlineAgent` is called with `projectId` set to the conversation's own id (not empty) so `ModelSelector`'s persisted settings (`chatProviderId`/`chatModelId`) get real per-conversation scoping — `agent-loop.ts`'s per-project memory-tool overlay explicitly excludes `agentName === "general-chat-assistant"` so this doesn't clobber the Assistant-exclusive memory tools with the generic per-project `agent_memories` binding. Don't re-add `run_shell` or file write/edit/delete/directory-browsing tools (or `take_screenshot`/`environment_info`) to `defaultAgentTools["general-chat-assistant"]` without re-confirming the workspace-less decision above still holds — those are gone by design, not by accident. `agent-loop.ts`'s `agentName !== "general-chat-assistant"` guards — on the auto "Workspace: ..." system-prompt injection, the `log_decision`/decisions-tool grant, AND the `pluginTools` merge — are all load-bearing for that same decision; removing any of them silently reintroduces workspace-path knowledge, a pointless `log_decision` tool, or whatever tools any currently-enabled (or future) plugin registers into Assistant. `GeneralChatContextIndicator` deliberately does NOT reuse the shared `ContextIndicator` component — that one reads `useChatStore`'s global `liveContextTokens`, which is shared with project chat in the same webview and could show a stale figure left over from a different conversation; General Chat's version estimates purely from its own local message list. No General-Chat-specific analytics are written; the app's own global AI-SDK telemetry sink (`ai_telemetry_events`) and the user's own prompt-log debug setting apply uniformly to every agent, this one included, by design — there is no per-agent opt-out. General Chat's turns run through `runInlineAgent` with `verifyToolCall: false` (a new passthrough option on `InlineAgentOptions`) — without it, the Claude Subscription CLI/SDK path's "at least one real tool call must land or the response is untrusted" guard (correct for sub-agent/Playground tasks, which are always concrete) misfired on ordinary zero-tool-call replies like "hi", surfacing a false "could not verify a real tool call" failure that PM chat/dashboard/collections/freelance/skills chat all already avoid via the same flag. General Chat is also the one caller that passes an explicit `streamingModeOverride` to `runInlineAgent` (`"full"` whenever the global Streaming setting is `"hybrid"`) rather than letting it fall back to `getStreamingMode()` — see the streaming-mode fix above; don't "simplify" this back to the default without re-checking that General Chat still streams live under Hybrid.
...
Voice ("Talk to PM") is a real cross-project assistant, not tied to whatever project chat happens to be open — see `docs/ambient-pm-voice-plan.md`. Mic is off by default; one tap starts a hands-free session. By default, speech input goes through the browser's Web Speech API with pause-based turn detection (a debounce timer, no button tap needed) that ends each turn automatically once the user stops talking; an alternative, opt-in **local/offline speech input pipeline** (continuous native mic capture → Silero VAD → Whisper small.en, all via `sherpa-onnx-node` + `node-cpal`, downloaded on demand — see the "Offline Ambient speech input" feature entry below) can be selected instead, giving each utterance a hard, VAD-determined boundary instead of a silence-timer guess. Either way, the session auto-restarts listening for the next turn — no "Ask again" tap needed — until the user explicitly taps "Stop". The mic is also kept alive (with a short arm delay for the Web Speech path) through the "thinking" (backend answering) and "speaking" (TTS playing) phases so the user can interrupt by talking, mirroring ChatGPT/Gemini-style voice mode; interrupting cancels any playing TTS immediately and the new speech starts the next turn, while the previous turn's still-in-flight backend call is actually cancelled server-side (`cancelAmbientAssistantTurn`, reusing `runAmbientAssistantTurn`'s existing `abortSignal` plumbing — the same mechanism the regular agent "Stop" button uses) rather than just having its eventual answer discarded — the interrupted turn is marked `interrupted` in the tool-call pane (an inline "— interrupted —" marker in place of an answer); deliberately no toast for this (tried once, explicitly reverted — a barge-in is routine, expected ambient behavior, not something worth a notification for every occurrence). A self-triggered cancellation (`controller.signal.aborted` in `runAmbientAssistantQuery`, rpc/ambient.ts) also resolves normally instead of throwing, so it never reaches `rpc-registration.ts`'s generic `withErrorToast` wrapper either — a genuine failure (provider error, timeout) still throws and still gets that toast, only the intentional-cancel case is silenced — see `docs/ambient-voice-barge-in-research.md` for the research behind this and why full automatic barge-in (safe even without headphones, i.e. also during the "speaking" phase) isn't attempted: it needs real acoustic echo cancellation, which neither speech-input path has today, so without headphones the mic can still occasionally pick up the assistant's own voice through speakers during "speaking" regardless of which recognizer is listening. The real one-shot cross-project tool-calling turn (`runAmbientAssistantTurn`) runs per turn and its answer is spoken **sentence-by-sentence as it streams in**, not held back until the whole response finishes generating — the biggest lever found for making this feel real-time, since TTS now overlaps whatever generation/tool-calls are still running. Works on any provider/model that streams token-level deltas (both the Claude Subscription CLI path's `onTextToken` and the regular `streamText` path's `text-delta` feed the same shared `handleTextDelta` in `assistant.ts`, which extracts complete sentences via `extractCompleteSentences` and fires a new `onTextChunk` callback per sentence, flushing any trailing incomplete one once the turn finishes); a provider/path that doesn't stream deltas simply never fires it, and `ambient-screen.tsx` falls back to speaking the complete answer in one shot exactly as before this feature — a pure addition, no regression for that case. Each chunk broadcasts via a new `ambientAssistantTextChunk` push event; the frontend queues and speaks them one at a time via a small drain loop, guarded by the same `isStillActive()`/`tts.cancel()` barge-in mechanism the rest of the turn already uses (no new per-chunk cancellation plumbing needed). The same deltas also feed a live-growing `"text"` part (throttled via `createThrottledAccumulator`, the same helper the Claude Subscription CLI path/PM chat/dashboard widgets already use for their own streaming) through the existing `ambientAssistantPart` mechanism, so the tool-call pane shows the answer forming — no frontend change was needed for that half; the pane's existing merge-by-id logic and answer-text render already handled a growing `content` generically. A rare CLI-only retry (`onRetract`, verification failure) resets this state so a discarded attempt's partial text/chunks don't bleed into the retry — not actually reachable for ambient today (`verifyToolCall: false`), but wired correctly regardless; the pane also reads the *last* `"text"` part rather than the first as a defensive match for that same edge case. If that's taking a while (700ms — `ACK_DELAY_MS` in `ambient-screen.tsx`, which comfortably skips a closing-remark reply's <20ms turnaround while still landing well before a real answer, even a plain no-tool-call one, given the Claude Subscription CLI subprocess's own 7s+ startup cost), a short fixed "one moment"-style ack phrase is spoken first (randomly one of `ACK_PHRASES` in `use-text-to-speech.ts`) so voice-first use isn't met with dead air through every tool call — gated on the turn still being active when the delay elapses, so an already-superseded turn never speaks up after the fact, and the real answer explicitly awaits the ack's own promise (never races/cuts it off) before speaking. The generated/offline voice path (`useAmbientVoicePlayback`) caches each ack phrase's audio per (provider, model, speed) the first time it's needed, replaying the cached clip on every later turn instead of re-paying that voice's 1–13s synthesis cost every time (the browser default voice has no generation cost regardless). A second, similarly-timed filler bridging the *generated-audio synthesis* gap itself (once the answer's text is back but its audio isn't rendered yet) was built and then explicitly dropped as not worth it — but building it surfaced a real, independently-worth-keeping bug fix: `generateAmbientSpeech` has no abort mechanism, so a `tts.cancel()` mid-generation couldn't actually stop that call — it would still resolve later and play its now-stale audio regardless. A `generationTokenRef` (`use-ambient-voice-playback.ts`, bumped on every `speak()`/`speakAck()`/`cancel()` call) now lets a superseded generation's late-arriving result be silently discarded instead of playing over whatever's current. Tools: `list_projects`/`get_project_status`/`list_active_agents`/`get_recent_activity`/`get_pending_approvals`/`get_review_queue`/`get_inbox_summary`/`get_scheduled_jobs`/`get_freelance_summary`/`get_git_status` (all read-only) plus `dispatch_to_project` (creates a new, persisted conversation in the named project and routes through that project's normal PM/plan-approval pipeline — never a bypass). Tool calls stream live into a slide-in side pane. Replies are spoken via `speechSynthesis` by default, via a real speech-model's generated audio, or via a downloadable offline voice ("Ryan", Piper/VITS via sherpa-onnx — fetched at download time into user-data, never bundled) if configured in Settings, at a configurable speaking speed (0.5x–2.0x, all three voice paths honor it). The local/offline speech-input pipeline (see its own feature entry below) merges consecutive VAD segments into one combined turn when the true audio-domain silence between them is short (≤~1100ms) rather than a natural mid-sentence thinking pause — without this, a normal composing pause (confirmed live, especially right after a barge-in, and worse than it first appeared once slow Whisper decodes were in the mix — see the feature entry) produced a second, independent turn that cancelled the first one instead of extending it; see `docs/ambient-voice-barge-in-research.md`'s merge-window notes for why a real silence measurement plus a decode-in-flight-aware backstop was chosen over a trained turn-detector model.
...
- `src/bun/ambient/assistant.ts` — `runAmbientAssistantTurn` (the one-shot cross-project tool-calling turn, dual-path model invocation mirroring `rpc/dashboard.ts`'s PM widget, up to 100 messages of conversation history); `handleTextDelta`/`extractCompleteSentences` feed both `onTextChunk` (sentence-by-sentence streaming TTS) and a throttled, progressively-growing pane `"text"` part from the same token-level deltas, shared by both the CLI and streamText model-invocation paths. System prompt is rebuilt fresh every turn (`buildSystemPrompt`, not a static string) — identity (`AgentDesk Project Manager`), current date/time (`loadUserTimezone` + the user's configured timezone), the shared `## User Profile` block (`buildUserProfileSection`), an `## Available Skills` listing (`skillRegistry.getAll()`), and `SECURITY_RULES_SECTION` — all reused directly from `agents/prompts.ts`, the same building blocks the main PM prompt and the dashboard widget use. Tools beyond the original cross-project status set: `search_projects` (fuzzy name match — voice transcription can mishear a project name), `read_skill`/`find_skills`/`list_skills`, `web_search`/`web_fetch`/`http_request`, `list_tasks`/`get_task`, `git_log`/`git_diff` (alongside the existing summary-only `get_git_status`; needs a project's `workspacePath`, which `list_projects`/`search_projects` now both return), `create_cron_job`/`list_cron_jobs` (voice reminders, not just read-only listing), `read_file`/`file_info`/`directory_tree`/`search_files`/`search_content` (project-scoped via `workspacePath`), `environment_info`/`get_env`/`check_process`/`sleep`, and `save_memory`/`recall_memory`/`delete_memory` — all reused directly from the same shared tool modules (`agents/tools/*`) the main PM and dashboard widget use, no reimplementation. The memory tools are backed by `agent_memories`, which requires a real project row (`projectId` is a NOT NULL foreign key) — since this assistant needs a memory scope of its own (not the shared `global_memories` pool, not any real project's PM memory), `ensureAmbientMemoryProjectId` creates/reuses one hidden pseudo-project (`__ambient_assistant_memory__`, `isQuickChat: 1` — same hiding mechanism Quick Chat uses, invisible to `list_projects`/Dashboard) purely to give `agentName: "ambient-assistant"` its own isolated memory rows; its always-on memory index is injected into the system prompt every turn via `buildMemoryIndexSection`, same as the main PM. `src/bun/ambient/tts.ts` — `generateAmbientSpeech` (takes an optional `speed` multiplier, forwarded to whichever adapter's `generateSpeech` is in play); `src/bun/ambient/local-voice-manager.ts` — the offline "Ryan" voice's download/status/preload/synthesize lifecycle (`synthesizeLocalVoice` also takes `speed`, forwarded to sherpa-onnx's own `generationConfig.speed`; fetched at download time into user-data, never bundled — see the "Offline Ambient voice" feature entry)
- `src/mainview/lib/use-global-agent-activity.ts` — `useGlobalAgentActivity`; `use-idle-timer.ts` — `useIdleTimer`; `use-ambient-settings.ts` — `useAmbientSettings`; `use-ambient-voice-turn.ts` — `useAmbientVoiceTurn` (pause-based auto-stop plus a `finalizing` flag covering the gap between calling `stop()` and the recognizer's async `onend`, wraps `use-voice-input.ts` without changing it for other callers); `use-local-stt-turn.ts` — `useLocalSttTurn` (same `{listening, finalizing, error, supported, toggle, stop}` shape as `useAmbientVoiceTurn`, so `ambient-screen.tsx` swaps between the two — `const voice = localSttActive ? localStt : rawVoice` — without touching any of the surrounding auto-restart/barge-in/voicePhase logic; each `ambientSttSegment` push is already a complete utterance, so it calls back directly rather than accumulating transcript state); `use-text-to-speech.ts` — `useTextToSpeech` (browser voice; `speak()` returns a Promise so a second utterance can be sequenced after the first, and resolves on `cancel()` too; `speakAck()`/`ACK_PHRASES` — the fixed rotating ack-phrase set); `use-ambient-voice-playback.ts` — `useAmbientVoicePlayback` (same shape as `useTextToSpeech`, swaps in generated/offline audio when configured; its `cancel()` also resolves any in-flight `speak()` promise, needed for barge-in to not hang; its own `speakAck()` caches each ack phrase's generated audio per (provider, model, speed) via `playGeneratedAudio`, a helper factored out of `speak()` so both share the same Audio-element/resolve wiring; a `generationTokenRef` discards a superseded `generateAmbientSpeech` call's late-arriving audio instead of playing it, for both `speak()` and `speakAck()`)
...
- `src/bun/rpc/ambient.ts` + `src/shared/rpc/ambient.ts` — `getAmbientDisplays`/`openAmbientDisplayWindow`/`closeAmbientDisplayWindow`/`getAmbientActivitySnapshot`/`getAmbientProjectionState`/`runAmbientAssistantQuery`/`cancelAmbientAssistantTurn`/`generateAmbientSpeech`/`getAmbientLocalVoiceStatus`/`downloadAmbientLocalVoice`/`preloadAmbientLocalVoice`/`getAmbientLocalSttStatus`/`downloadAmbientLocalStt`/`startAmbientLocalListening`/`stopAmbientLocalListening` (`ambientAssistantPart` push event streams tool-call/text parts live, including a progressively-growing `"text"` part while the answer streams in; `ambientAssistantTextChunk` streams one complete sentence at a time as the answer generates, for sentence-by-sentence TTS; `ambientLocalVoiceStatus`/`ambientLocalSttStatus` push events stream the two offline downloads' progress; `ambientSttSegmentStart` fires the instant VAD detects a segment (before decode begins), then `ambientSttSegment` (carrying `silenceBeforeMs`) streams each VAD-bounded utterance once Whisper decodes it)
...
- Settings: `ambientModeEnabled`/`ambientModeIdleMinutes`/`ambientModeVoiceEnabled`/`ambientModeTtsEnabled`/`ambientTtsProviderId`/`ambientTtsModelId`/`ambientTtsSpeed`/`ambientSttProviderId` in `src/mainview/pages/settings/ambient.tsx` — its own top-level Settings tab (`settings.tsx`, right after "Integrations"), split out of General once Ambient Mode's own settings grew large enough to warrant it (the TTS voice picker reuses the existing `getConnectedProviderModels`/`getModelTypes` RPCs the Models settings page already uses for type badges; the STT picker is a plain two-option Select — no per-model catalog, since there's only ever the one bundled local model)
...
### Offline Ambient speech input (local VAD + Whisper, downloaded on demand)
An alternative, opt-in speech-input pipeline for Ambient Mode's "Talk to PM" — Settings → General "Speech input" picker, default is the existing Web Speech API path. Selecting "Local (offline, continuous listening)" downloads a mic-capture library (`node-cpal`, native Rust CPAL bindings — WASAPI/CoreAudio/ALSA) plus a Silero VAD model plus a Whisper small.en model (all via `sherpa-onnx-node`, reusing the same engine already downloaded for the "Ryan" TTS voice if present, instead of a second copy). Once running, mic capture is continuous and gated by VAD: each detected utterance is a hard-bounded segment (not a silence-timer guess) decoded by Whisper (in a dedicated worker thread, not the main thread — see below) and streamed to the frontend as a complete transcript — replacing `useAmbientVoiceTurn`'s transcript-accumulation/pause-timer logic entirely for this path, since VAD already knows definitively when speech starts and stops. Built and model-compared live (see `docs/ambient-voice-barge-in-research.md`'s follow-up research): Whisper small.en was chosen over Moonshine-tiny/-base and Whisper-tiny/-base after live accuracy comparison on real (not synthesized) speech — smaller models measurably mis-transcribed common words. This only replaces the *speech-to-text* layer — it does not add real acoustic echo cancellation, so the same "no barge-in during TTS playback without headphones" limitation as the Web Speech API path still applies (see the Ambient Mode entry above).

Whisper decode measured 1.3–10s per segment live, and a live log review (2026-07-20, see `docs/ambient-voice-barge-in-research.md`) found that decoding it synchronously on the main thread broke multi-segment utterances: a single continuous thought with normal thinking pauses fragmented into several separate turns, each barge-in-cancelling the last, because the merge-window fix (below) was comparing decode-latency-skewed arrival times instead of the user's actual pause length. Fixed with two changes working together: (1) decode now runs in a persistent worker thread (`getDecodeWorker`/`decodeInWorker`, worker source written to disk at runtime and loaded via `new Worker(pathToFileURL(...))`, mirroring `db/maintenance.ts`'s `runVacuumInWorker` pattern but long-lived) so the main thread keeps accepting waveform/detecting new segments in real time regardless of decode duration; (2) each segment now carries `silenceBeforeMs` — the true audio-domain gap since the previous segment, computed from VAD's own sample-accurate `start` index, immune to decode latency — plus a new `ambientSttSegmentStart` push event fired the instant VAD detects a segment (before decode begins), so the frontend's merge/flush logic (`use-local-stt-turn.ts`) can tell "still transcribing a continuation" apart from "genuinely done talking" without guessing a fixed timeout.

Continuous mid-sentence pauses (VAD's own `minSilenceDuration` is 0.4s, tuned for segmentation, not for "done talking" vs. "still thinking") — a natural composing pause, especially right after a barge-in, reliably exceeds that and used to produce a second independent VAD segment that `processTurn` treated as a fresh interruption. `silenceBeforeMs` ≤ ~1100ms now merges into the pending utterance instead; a real pause beyond that (or nothing pending) flushes/starts fresh immediately, since the true gap already settles the question — no need to wait. The tail-end "is anything else coming" question (no next segment exists yet to measure against) uses a fixed backstop (1400ms) after the last decode completes, but `ambientSttSegmentStart` — edge-triggered off `vad.isDetected()` flipping true, i.e. within ~0.25s of the user actually resuming speech, not once a whole continuation is already spoken and popped off VAD's queue (a first version fired it there, and a second live-log review caught that it was too late to reliably save a fast-decoding segment from the backstop) — keeps that backstop from firing while a continuation is still being spoken/transcribed, however long that takes.
**Key files:**
- `src/bun/ambient/local-stt-manager.ts` — `LOCAL_STT_PROVIDER_ID`, `downloadLocalStt`/`getLocalSttStatus`/`startLocalListening`/`stopLocalListening`/`isLocalSttReady`; reuses `local-voice-manager.ts`'s TTS engine directory read-only if present (checked by path, no import coupling), else downloads its own copy; `getDecodeWorker`/`decodeInWorker` run Whisper inference in a background worker, VAD/resampling stay on the main thread
- `src/bun/rpc/ambient.ts` — `getAmbientLocalSttStatus`/`downloadAmbientLocalStt`/`startAmbientLocalListening`/`stopAmbientLocalListening`; `startAmbientLocalListening` broadcasts `ambientSttSegmentStart` the instant VAD detects a segment, then `ambientSttSegment` (with `silenceBeforeMs`) once it's decoded
- `src/mainview/lib/use-local-stt-turn.ts` — `useLocalSttTurn` (silence-gap-based merge + decode-in-flight-aware flush backstop); `src/mainview/pages/settings/general.tsx` — reuses the same `LocalVoiceDownloadPanel` component as the TTS voice card
**Data:** none (marker file only, no DB row) — reuses the `ambient_stt_provider_id` settings key (new) alongside the existing `ambient_tts_provider_id`/`ambient_tts_model_id` pattern
**Watch for:** Whisper's archive from sherpa-onnx bundles both fp32 and int8 weights — only the int8 files are loaded, and the fp32 copies are deleted right after extraction (`pruneNonInt8Whisper`) to avoid ~970MB of wasted disk. `node-cpal` is an early-stage (0.1.x), less-mature dependency than `sherpa-onnx-node` — validated live via a throwaway spike (`demo/stt-vad-spike/`) before building this, but worth remembering if mic-capture issues surface later. The continuous native capture session has no live error-recovery path back to the frontend if it fails mid-session (only start-time errors surface today) — an accepted v1 gap, not a design decision to defend indefinitely. The decode worker is long-lived (reused across start/stop cycles like `cachedEngine`/`cachedRecognizer`), not torn down on `stopAmbientLocalListening` — intentional (avoids repaying its own recognizer-load cost every session) but means it stays resident in memory once first used in a given app run.
...
The global Settings page (`src/mainview/pages/settings.tsx`) renders a top-level `Tabs` (General, AI, Channels, Integrations, Ambient, Notifications, System, Plugins), each holding a `SubTabs` of individual setting pages (Ambient and Notifications are single pages with no sub-tabs). A separate `aiOnly` mode renders just the AI sub-tabs directly (no outer tab chrome) for Quick Chat windows, since a Quick Chat "project" has no kanban/plan-approval/ambient/channels/plugins to configure.
**Key files:**
