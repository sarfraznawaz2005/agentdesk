...
| `src/bun/agents/tools/skills.ts` | Skills tools: `read_skill`, `read_skill_file`, `find_skills`, `list_skills`, `validate_skill` |
...
**Fixed: the global Streaming setting had no visible effect on General Chat.** Turned out General Chat's live text was never rendered at all, in any mode — `InlineAgentCallbacks.onTextDelta` (what `general-chat.tsx`'s `streamingText` state was wired to) is genuinely never called anywhere in `agent-loop.ts`; it's only invoked by PM chat's/the six widget-chat surfaces' own separate direct-`streamText` loops, not by `runInlineAgent`. The real live-text mechanism `runInlineAgent` provides is part-based: in `streamingMode === "full"`, `pushLiveDelta` creates/updates a `type: "text"` message part as tokens arrive; General Chat's `onPart` handler only ever handled `type: "tool_call"`, and `generalChatPartUpdated` had no listener at all. Fixed on two levels: (1) `general-chat.tsx` now handles `"text"` parts too — tracks the live part's id (`streamingTextPartIdRef`) and a `streamingCommittedTextRef` prefix so a later step's fresh part (e.g. "let me check that…" → tool call → final answer, each step gets a new part id) appends onto what's already shown instead of blanking it; a real `agentdesk:general-chat-part-updated` listener was added (`onPartUpdated`) to consume the DB-parity content updates `pushLiveDelta` actually sends. (2) General Chat has no sub-agent-card concept, so Hybrid's entire reason to differ from Full never applied to it. It now has its OWN streaming setting entirely decoupled from the global one — a three-mode (`"full"` smooth / `"chunked"` larger blocks / `"none"`) preference in a **dialog opened by the gear icon in the General Chat header** (`components/general-chat/general-chat-settings.tsx`), persisted under key `generalChatStreamingMode` (category `ai`) and read by `getGeneralChatStreamingMode` (`agents/streaming-mode.ts`). All three of General Chat's modes are valid `StreamingMode` values, so `orchestrator.ts` passes the choice straight through as `streamingModeOverride`; `runInlineAgent`'s `pushLiveDelta` derives the flush cadence from it centrally via `streamingFlushArgs` (`"chunked"` → coarse 500ms + newline-flush), so code/lists stream line-by-line while unbroken prose updates ~twice a second with far fewer frontend re-renders. Default is `"chunked"` — the constant re-render churn of smooth per-token streaming was the main source of perceived lag on long General Chat replies. Existing users (who never set `generalChatStreamingMode`) pick up the new default automatically, no migration. See `streaming-mode.ts`'s doc comments and the Settings → AI → Streaming page (which now notes General Chat has its own setting). **Note:** `"chunked"` was later also added to the *global* streaming setting (see the Streaming mode feature entry in feature-list.md) — it reuses the same `isLiveStreamingMode`/`streamingFlushArgs` helpers, but for the global PM/widget surfaces it's applied on the PM chat frontend token buffer and the widget backend accumulators respectively, since those don't all go through `runInlineAgent`. The pre-existing `onTextDelta`/`generalChatTextDelta` plumbing was left in place (harmless dead code) rather than removed — it's a wider gap affecting every `runInlineAgent`-based caller (Playground, sub-agent cards, issue-fixer, etc.), out of scope for a General-Chat-specific fix.
