...
- **New `src/bun/general-chat/orchestrator.ts`**:
  ...
  - On turn completion: persist exactly two rows to `generalChatMessages` — the final user message and the final assembled assistant text — then `broadcastToWebview("generalChatComplete", …)`.
