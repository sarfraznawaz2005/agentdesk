...
3. `run_agent` dispatches the sub-agent **fire-and-forget** and immediately tells
   the PM to stop its stream (`pm-tools.ts:578`, `:793`). The PM's turn ends here —
...
(`src/bun/agents/engine.ts:47`) and the standalone review cycle exist. The
...
- **Soft / semantic transitions → the system prompt.** The Plan → Approve →
  ...
  natural-language instructions the PM follows (`prompts.ts:310` classification,
  `prompts.ts:340` complex-task workflow, `prompts.ts:369` kanban flow). Because
...
- **Hard invariants → code guards in the tools.** Anything that must never happen
  ...
  (`pm-tools.ts:250`, `:369`), a module-level `dispatchingAgents` set to close the
  parallel-tool-call race (`pm-tools.ts:241`, `:346`), plan-mode read-only
  restriction (`pm-tools.ts:361`), and a block on dispatching while a task is in
  `review` (`pm-tools.ts:392`). Kanban column transitions are likewise enforced in
  code, not prompt — the agent cannot move a task to `done`; only the review system
  does (`prompts.ts:372`, see [[agent-engine]] / review-cycle).
...
- **The LLM is already a planner.** A separate FSM would duplicate reasoning the
  ...
  `prompts.ts:310`) and dependency-aware task ordering are exactly what an LLM is
...
- **Concurrency is structurally simple.** Write work is strictly sequential
  ...
  the only "lock" needed is `writeAgentRunning` (`pm-tools.ts:250`). Read-only
  agents fan out via `run_agents_parallel` but produce no state mutations
  (`pm-tools.ts:819`). An FSM's value is mostly in coordinating concurrent
...
- **Restart-safe by reconstruction, not by persistence.** Because the next action
  ...
  (`engine.ts:415`) and `get_next_task` (`pm-tools.ts:1760`), there is no FSM state
...
| `src/bun/agents/prompts.ts` | `PM_PROMPT_TEMPLATE` (`:270`) — the orchestration "rules" in natural language: classification, Plan→Approve→Execute, kanban flow |
