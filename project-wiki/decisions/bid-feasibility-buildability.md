...
system prompt at `src/bun/rpc/freelance-wizard.ts:416`):

- **Condition A — System check:** every runtime/tool/dependency the project needs is
  *confirmed installed* on the local dev box (`freelance-wizard.ts:422`). This is
  ...
  ("fail safe", `freelance-wizard.ts:432`).
- **Condition B — AI capability:** the agent roster can fully deliver the technical
  requirements (`freelance-wizard.ts:435`). The prompt enumerates what the AI *can*
  ...
  `freelance-wizard.ts:439-441`.

If either fails, the verdict is NOT WORKABLE (`freelance-wizard.ts:420`).
...
(`freelance-wizard.ts:443-448`) so the model does not reject buildable jobs for
...
`analyzeListingWorkability` (`freelance-wizard.ts:656`) runs the analysis in two
...
1. **Phase 1 — verify + analyse:** `generateText` with the wizard tool subset
   (`freelance-wizard.ts:171`) and `toolChoice: "auto"`. A `TOOL_DIRECTIVE`
   (`freelance-wizard.ts:514`) forces `environment_info` + per-runtime `run_shell`
   version checks before any prose. If the model skips the checks, they are forced
   (`freelance-wizard.ts:693`).
2. **Phase 2 — structured extraction:** a strict data-extractor pass turns the prose
   ...
   (`freelance-wizard.ts:747`), parsed by `extractJsonFromText`
   (`freelance-wizard.ts:621`) and normalised by `coerceVerdict`
   (`freelance-wizard.ts:580`). Tellingly, when the model omits the boolean,
   `coerceVerdict` derives `workable` from `coveragePercent >= 95`
   (`freelance-wizard.ts:586`) — the bar is essentially "100% buildable", matching
   ...
   *concrete missing tools or AI limitations* as blockers, never "incomplete
   analysis" (`freelance-wizard.ts:756`).

The verdict is persisted to `freelance_listings.wizard_verdict / wizard_reason /
wizard_blockers / wizard_analysis_text` (`freelance-wizard.ts:1042`).
...
as stale via `isStaleGateVerdict`, `freelance-wizard.ts:126`):

- **Skill gate** (`skillGateBlocks`, `freelance-wizard.ts:140`): Freelancer.com only
  lets you bid when your profile shares ≥1 skill with the project. Zero overlap →
  blocked; unknown profile or no listed skills → **fail open** (`:144`, `:149`,
  `:153`). This is a platform mechanic, not buildability. See
  [[freelancer-skill-gate]].
- **Client-quality gate** (`freelance-wizard.ts:313`+): filters on the user's own
...
already deemed workable. `draftBidForListing` (`bid-pipeline.ts:51`) fetches the full
listing description (`ensureFullDescription`, `bid-pipeline.ts:67`), writes the
proposal with a winning-bid system prompt (`buildProposalSystem`,
`bid-pipeline.ts:22`), runs a QA revise pass and a draft-time near-duplicate guard
(`bid-pipeline.ts:81-96`), then enqueues a `bid` draft to `freelance_outbox`
(`bid-pipeline.ts:101`). Note: the pipeline itself does **no** budget/credential/
seniority checking — consistent with this decision, those never gate a bid. The
send-time [[freelance-behavior-governor]] (caps, dedup, pacing) is the backstop.
...
- **`workable` collapses to a near-100% coverage threshold** when the model omits the
  boolean (`coverage >= 95`, `freelance-wizard.ts:586`). Partial-coverage jobs are
  rejected by design.
...
## Related
- [[freelancer-skill-gate]]
- [[freelance-behavior-governor]]
- [[auto-earn]]

## Open questions
- The "writing" prompt (`buildAnalysisWritePrompt`, `freelance-wizard.ts:530`) and
  the heuristic fallback when verdict JSON fails to parse (`freelance-wizard.ts:769`)
