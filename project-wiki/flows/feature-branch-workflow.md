...
`"true"` or `autoCommitTask` returns immediately (`review-cycle.ts:376-378`).
...
`getPMSystemPrompt` calls `isFeatureBranchWorkflowEnabled` (`prompts.ts:855`)
and, when true, appends `FEATURE_BRANCH_SECTION` (`prompts.ts:869-882`) to the
PM system prompt (appended at `prompts.ts:982`). That section is what *instructs* the PM to call
...
Write sub-agents get a much terser instruction (`prompts.ts:1245-1246`): "never
...
This is the mechanical half, in `review-cycle.ts:374`. It is invoked when a task
reaches "review" — from two call sites: the kanban `submit_review` path
(`kanban.ts:738`) and the PM `run_agent` completion path (`pm-tools.ts:655-656`).

When `featureBranchWorkflow` is `"true"` (`review-cycle.ts:393-394`):

1. Read `currentFeatureBranch:<projectId>` (`review-cycle.ts:396`).
2. Choose the branch: the stored PM name if it starts with `feature/`,
   **otherwise** a slug derived from the task title (`review-cycle.ts:397-398`).
...
3. Compare to the current branch (`review-cycle.ts:401-404`); if already there,
   do nothing.
4. Otherwise `git checkout <branch>` if it exists, else `git checkout -b
   <branch>` to create it from the current HEAD (`review-cycle.ts:409-416`).
...
the diff (`review-cycle.ts:420-458`). The whole function is wrapped to swallow
errors — branch/commit failures are logged, never thrown
(`review-cycle.ts:463-465`).
...
- **Two prerequisites, not one.** Branch switching only happens when *both*
  `autoCommitEnabled` (`review-cycle.ts:378`) **and** `featureBranchWorkflow`
  (`review-cycle.ts:394`) are `"true"`. The feature is a layer on top of
  auto-commit.
- **Inconsistent truthy checks.** `review-cycle.ts:394` matches only the literal
  string `"true"`, while `isFeatureBranchWorkflowEnabled` accepts both `"true"`
  and the double-quoted `'"true"'` (`prompts.ts:862-863`). Because `getSetting`
...
- **Silent title-slug fallback.** If the PM skips `set_feature_branch`, each
  ...
  (`review-cycle.ts:398`), defeating the "one branch per feature" intent — there
...
- **No automatic PR / push / merge.** The workflow only switches branches and
  ...
  driven purely by the prompt (`prompts.ts:881`), not by code.
...
## Open questions
- Should `review-cycle.ts:394` adopt the same `'"true"'` tolerance as
  `prompts.ts:863` to avoid the double-encoding mismatch, or is the JSON-parsing
