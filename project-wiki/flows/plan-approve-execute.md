...
1. **The plan card must be the real plan doc, not a PM paraphrase.** `request_plan_approval`
   ...
   instead (`pm-tools.ts:1624`). The PM tends to summarise lossily; the task-planner's
...
2. **The LLM cannot be trusted to re-list tasks.** `create_tasks_from_plan` never re-runs
   ...
   `define_tasks` (`pm-tools.ts:1741`, `planning.ts`). A second inline planning run reliably
   truncates the list.
3. **The PM turn must end while waiting for a human.** Approval is not a tool that blocks; it
   ...
   `deps.stopPMStream?.()` (`pm-tools.ts:1647,1702`) which flips `planApprovalRequested` in
   the engine (`engine.ts:398`), breaking the PM stream loop (`engine.ts:705`). The PM only
...
- **PM-driven:** the PM calls `request_plan_approval` (`pm-tools.ts:1604`). In-app it persists
  a `type:"plan"` message and broadcasts `planPresented` (`pm-tools.ts:1695`); for
  `channel:` conversations it instead chunks the plan into a WhatsApp/Discord/Email message
  (`pm-tools.ts:1641`). Either way it calls `stopPMStream`.
- **Code-enforced:** because the Vercel AI SDK runs parallel tool calls via `Promise.all`,
  ...
  call is rejected (`pm-tools.ts:1665`). The card is instead shown deterministically inside the
  `run_agent` completion handler when `agent === "task-planner"` and there are pending defs
  (`pm-tools.ts:689-770`). This guarantees the card appears even if the PM forgets to ask, and
  prevents the PM from rendering "approval" as plain chat text.

A guard at `pm-tools.ts:701` suppresses the card if any non-`done` kanban task already exists
...
(`pm-tools.ts:770`) — it only inserts an assistant "context" message recording the plan
...
Before dispatching write-agents, the PM calls `set_feature_branch` (`pm-tools.ts:2695`). It
reads the last 5 user messages, asks the LLM for a `feature/<slug>` name, collects existing
branch + PR source-branch names to avoid collisions (`pm-tools.ts:2727`), validates the format,
and persists it to `settings` under `currentFeatureBranch:<projectId>` (category `git`,
`pm-tools.ts:2766`). `autoCommitTask` in [[agent-engine]]'s review cycle later checks out / creates
this branch before committing. `clear_feature_branch` wipes it after the PR (`pm-tools.ts:2774`).

### 5. Create tasks (deterministic)
`create_tasks_from_plan` (`pm-tools.ts:1715`) drains the stored definitions and creates one
kanban task per def in the `backlog` column, resolving each def's `blocked_by` *indices* into
real task IDs by position (`pm-tools.ts:1754`). No LLM runs here — it is a pure replay.
...
set to defeat duplicate parallel dispatches from a single LLM step (`pm-tools.ts:346-363`).
Read-only agents bypass this. The task auto-moves `working -> review` on completion only if
`verify_implementation` passed (`pm-tools.ts:642`); otherwise it stays in `working` with a
warning appended to the result. `get_next_task` (`pm-tools.ts:1784`) picks the next unblocked
...
| `src/bun/agents/engine.ts` | PM streaming loop; `stopPMStream` -> `planApprovalRequested` breaks the loop and ends the turn (`engine.ts:398,705`) |
...
- **Don't `onAgentDone` after task-planner.** The completion handler deliberately returns early
  (`pm-tools.ts:770`) so the PM does not restart and re-plan before the user sees the card.
- **`request_plan_approval`'s `summary` arg is usually discarded.** The card content is the most
  recent Docs note, not the PM's summary (`pm-tools.ts:1624`); summary is only a fallback.
- **Task definitions are in-memory, not in the DB.** They live in `planning.ts`'s buffer. An app
  ...
  "No task definitions found" (`pm-tools.ts:1743`).
...
- **Channel vs in-app branch everywhere.** `conversationId.startsWith("channel:")` toggles
  ...
  `run_agent` / `create_tasks_from_plan` (`pm-tools.ts:311,1727`).
