...
// such an agent would wrongly collapse it to only that one tool. Agents with no rows
// (e.g. freelance-expert, issue-fixer, playground) already expose the tool via the
// registry overlay, so they need no backfill. Idempotent — agents that already have
// the row are skipped.
