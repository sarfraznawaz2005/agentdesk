// ---------------------------------------------------------------------------
// App-wide secret encryption at rest (AES-256-GCM)
// ---------------------------------------------------------------------------
// Canonical implementation shared by every feature that stores a credential in
// the SQLite DB (Remote Sync passwords, per-project GitHub tokens, issue-tracker
// API keys, …). The 32-byte master key is generated once and stored in a file
// under userData — SEPARATE from the DB — so a leak of agentdesk.db alone does
// not expose secrets.
//
// The key file is named `remote-sync.key` for historical continuity: Remote Sync
// shipped this scheme first, so reusing the same key keeps already-encrypted
// Remote Sync credentials decryptable. It now protects all app secrets.
//
// Not OS-keychain-backed (no native keychain binding on the Bun runtime); this
// raises the bar meaningfully over plaintext-in-DB.
// ---------------------------------------------------------------------------
