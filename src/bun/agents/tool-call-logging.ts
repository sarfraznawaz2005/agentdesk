	...
	for (const [name, t] of Object.entries(tools)) {
		...
		(wrapped as Record<string, Tool>)[name] = {
			...
			execute: async (args: unknown, execOpts: unknown) => {
				...
				console.log(`[TOOLCALL] agent=${actorLabel} tool=${name} args=${JSON.stringify(args ?? {}).slice(0, 300)}`);
				try {
					const result = await orig(args, execOpts);
					console.log(`[TOOLCALL DONE] agent=${actorLabel} tool=${name} durationMs=${Date.now() - start}`);
					return result;
				} catch (err) {
					console.log(`[TOOLCALL ERROR] agent=${actorLabel} tool=${name} durationMs=${Date.now() - start} error=${err instanceof Error ? err.message : String(err)}`);
					throw err;
				}
			},
		} as Tool;
