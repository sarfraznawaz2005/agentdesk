...
export async function runPlayground(userMessage: string, consoleErrors?: string[]): Promise<void> {
	...
	try {
		...
		const result = await runInlineAgent({
			...
			priorMessages,
			extraTools,
			// request_human_input IS allowed now — it pops the in-app dialog + desktop notification
			// (5-min wait). chrome-devtools_* MCP tools attach to a separate external browser and
			// can't see the in-app preview, so the Playground agent must never use them.
			excludeTools: ["chrome-devtools_*", "verify_implementation"],
