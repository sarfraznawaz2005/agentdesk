...
export async function runPlayground(userMessage: string, consoleErrors?: string[]): Promise<void> {
	...
	try {
		...
		const result = await runInlineAgent({
			...
			priorMessages,
			extraTools,
			// request_human_input would block with no clear answerer for this build loop. chrome-devtools_*
			// MCP tools attach to a separate external browser and can't see the in-app preview, so the
