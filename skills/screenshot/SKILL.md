...
3) If you need a screenshot for your own inspection (not to show the user), save to the temp directory.
...
The helpers route Swift's module cache to `$TMPDIR/agentdesk-swift-module-cache`
to avoid extra sandbox module-cache prompts.

```bash
bash ${AGENTDESK_SKILL_DIR}/scripts/ensure_macos_permissions.sh
```
...
```bash
bash ${AGENTDESK_SKILL_DIR}/scripts/ensure_macos_permissions.sh && \
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --app "<App>"
```

For your own inspection runs, keep the output in temp:

```bash
bash ${AGENTDESK_SKILL_DIR}/scripts/ensure_macos_permissions.sh && \
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --app "<App>" --mode temp
```
...
```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py
```
...
- Temp location (agent visual check, not shown to the user):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --mode temp
```

- Explicit location (user provided a path or filename):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --path output/screen.png
```
...
```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --app "<App>" --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --list-windows --app "<App>"
```

- Pixel region (x,y,w,h):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --mode temp --region 100,200,800,600
```

- Focused/active window (captures only the frontmost window; use `--app` to capture all windows):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --mode temp --active-window
```

- Specific window id (use --list-windows on macOS to discover ids):

```bash
python3 ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.py --window-id 12345
```
...
```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1 -Mode temp
```

- Explicit path:

```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1 -Path "C:\Temp\screen.png"
```

- Pixel region (x,y,w,h):

```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1 -Mode temp -Region 100,200,800,600
```

- Active window (ask the user to focus it first):

```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1 -Mode temp -ActiveWindow
```

- Specific window handle (only when provided):

```powershell
powershell -ExecutionPolicy Bypass -File ${AGENTDESK_SKILL_DIR}/scripts/take_screenshot.ps1 -WindowHandle 123456
```
...
- On macOS, run `bash ${AGENTDESK_SKILL_DIR}/scripts/ensure_macos_permissions.sh` first to request Screen Recording in one place.
