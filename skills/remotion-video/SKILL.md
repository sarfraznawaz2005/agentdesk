---
name: remotion-video
description: Create, preview, and render real videos with Remotion (video-in-React). Use whenever the user wants to make, edit, animate, or render a video — motion graphics, promos, intros/outros, explainers, captioned/subtitled clips, social posts, slideshows — or works with Remotion code, compositions, or .tsx video components. Trigger on requests to "make/generate/render a video" or anything Remotion-related.
agent: frontend_engineer
allowed-tools: run_shell, read_file, write_file, read_skill_file
metadata:
...
This skill builds real videos with Remotion — a **Node.js + React** framework. Check this before scaffolding or rendering:

- **Node.js (with `npx`) is required.** Detected version:

  !`node --version || echo NOT INSTALLED`

  If that reads `NOT INSTALLED`, stop and tell the user to install Node.js from
  **Settings → System → Recommendations → Node.js (Install)**, then retry. Do not
  try to scaffold or render a video without Node.
- **No separate FFmpeg or browser install is needed** — Remotion ships FFmpeg via
  `npx remotion ffmpeg` / `ffprobe` and auto-downloads its rendering browser on the
  first render (one-time, needs network).
- Remotion + React install **into the video project folder** via the scaffold
  command below — nothing global is touched.
