...
- **Issue Fixer** (`src/bun/issue-fixer/`): Per-project autonomous GitHub issue resolution. A 60s poller (`poller.ts`, started in `index.ts`) polls each enabled project's GitHub issues + comments (outbound — no inbound webhooks, NAT-safe) at its configured interval. A trigger fires only when an `agentdesk-*` keyword matches an issue **title** or an authorized **comment** (never the body), or an `agentdesk-*` **label** is present, AND the actor is authorized (OWNER/MEMBER/COLLABORATOR, or label-gated) — see `triggers.ts`. The dedicated hidden `issue-fixer` agent (full registry incl. chrome-devtools MCP + git tools + guarded auto-shell; `request_human_input`/`git_push`/`git_pr` excluded) runs via `runInlineAgent` (`orchestrator.ts`, sequential per-project queue, registers an agent controller so it shows on the dashboard card) — **no PM, no kanban, no approval card**. The orchestrator deterministically creates an `issue-fix/<n>-<slug>` branch, runs the agent, runs the test/build gate, commits, pushes (token-authenticated via `pushBranchAuthenticated` — auth supplied via an inline header with the credential helper **disabled**, never embedded in the push URL, so git never stores an `x-access-token` account in the user's Git Credential Manager), and opens a PR (`Fixes #N`, draft if tests fail or autonomy=draft). **It NEVER merges — humans merge** (enforced in the agent prompt, by excluding `git_pr`/`git_push`/`git_reset`/`git_cherry_pick`/`git_branch`/kanban-write tools, and by `shell-guard.ts`'s denylist which blocks merges, force-push, base-branch push, and all destructive/undo commands — `reset`/`clean`/`restore`/`checkout`/`switch`/`branch -D`/`push --delete`/history-rewrite/recursive `rm`). Runs require a clean working tree (never stashes the user's changes), reuse an existing branch on re-trigger (never `-B` reset), and skip empty PRs. The orchestrator operates in the **real project workspace** (not a worktree), so it **records the branch the user was on at start and checks it back out in `finally`** — the user is never silently left on the `issue-fix/*` branch (the fix is safe on the branch+remote+PR; restore runs with no abort signal so it survives a Stop). On the success path the tree is already committed (clean switch); on a FAILED run the agent's leftover uncommitted/untracked edits (guaranteed to be this run's, since a clean tree is required at start) are first **stashed** (labelled, recoverable) before the checkout — so a plain checkout can't carry them onto the user's branch and the return always succeeds. A maintainer's `agentdesk-*` comment on the agent's PR updates that PR's branch (PR-feedback loop). Config + run history live in `issue_fixer_config` / `issue_fix_runs` (surfaced as the **Git → Issue Fixer** sub-tab, which itself has three sub-tabs — Activity / History / Configuration — with the config form in Configuration; live Activity + History stream via `agentdesk:issuefixer-*` broadcasts). Summaries go to all connected channels on success+failure (`notify.ts`). Intent keywords: `agentdesk-fix/feature/test/docs/refactor/review` (`prompts.ts`).
- **Remote Sync** (`src/bun/remote-sync/`): Per-project **Remote** tab (sibling of Git/Deploy in `project.tsx`) for projects whose files live on an SFTP/FTP(S) server with no git. A protocol-agnostic `RemoteClient` (`client.ts`) wraps `ssh2-sftp-client` (SFTP, password or private-key auth) and `basic-ftp` (FTP/FTPS) — both validated on Bun 1.3.x. Users browse a **lazy remote tree** (checkbox selection, `browseRemoteDir` per expand), **Pull** the selection into the project `workspacePath` (so agents work on the files with normal file tools), and **Push** changes back through a **diff preview** (new/modified uploadable, deletions reported but never applied remotely). Change detection uses a local↔remote manifest (`remote_sync_items`: remote size/mtime + local SHA-256 at last sync). The diff also **checks the server** (one directory listing per folder — rsync/FileZilla style, not per-file stat) and flags any candidate whose remote copy changed since the last sync (or a "new" file that already exists remotely) — those `remoteChanged` entries are left unchecked so a push can't silently clobber someone else's edit. Each row has an expandable **diff** (server copy vs local edit) rendered by the shared `components/ui/unified-diff.tsx` `UnifiedDiffCard` (extracted from the chat tool-call card — same renderer for agent edits and push previews; `getRemotePushFileDiff` fetches both sides, binary/large files skipped). Folder-tree browsing reuses a **short-lived per-project connection cache** (idle-evicted ~30s, serialized, invalidated on config save) instead of reconnecting per expand. Credentials are **encrypted at rest** (AES-256-GCM; master key in a file under userData, separate from the SQLite DB — `crypto.ts`); the frontend only ever sees presence booleans, never cleartext secrets. The `engine.ts` enforces one operation per project at a time (AbortController lock), streams per-file progress + a log via `agentdesk:remotesync-*` broadcasts, and records history in `remote_sync_runs`. It **never deletes remote files**. Config/manifest/runs persist in `remote_sync_config` / `remote_sync_items` / `remote_sync_runs` (migrations v29 + v30). Security: FTPS TLS verification is a per-project toggle (`rejectUnauthorized`, default off to tolerate self-signed internal-server certs); SFTP **pins the server host key** trust-on-first-use (`hostKeyFingerprint` via ssh2 `hostVerifier`, SHA-256) and refuses a changed key (mismatch surfaced; "Forget / re-trust" in the UI clears it). **Exclude patterns** (`excludePatterns`, glob; name-without-slash matches at any depth, e.g. `node_modules`/`*.log`) prune both pull walks and push diff (explicit file selections are never excluded). SFTP private-key auth takes the **pasted PEM contents** (no file picker); saved password/passphrase can be revealed on demand (`revealRemoteSyncSecret`, eye icon). The connection form uses standard FTP-client fields incl. a **Remote directory** (starting directory) + **Local directory**. Symlinks are skipped (loop-safe); download paths are containment-checked against the workspace root (path-traversal-safe). FTP listing does **CWD-then-LIST** (FileZilla-style — many servers ignore the path arg to MLSD/LIST); the FTP client sniffs the server's "226 N matches total" reply to detect a **failed data transfer** (reported > 0 but parsed 0 — typical of plain-FTP data channels mangled by a firewall/ALG), retries once, and otherwise raises a clear "switch to FTPS" error instead of a silent empty listing. (Verified against a real PureFTPd host: plain FTP data channel flaky, **FTPS 100% reliable**.)
